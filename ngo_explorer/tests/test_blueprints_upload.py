...
def test_upload_process(client, m):
    rv = client.post(
        '/upload/', data={"charitynumbers": "225922,301020"})
    assert rv.status_code == 303
    rv = client.get(rv.headers["Location"])
    assert b'Data on 2 charities' in rv.data
    
    rv = client.post(
        '/upload/', data={"upload-name": "Test Upload 1234", "charitynumbers": "225922,301020"})
